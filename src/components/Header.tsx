        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-start w-1/3 space-x-2">
            ...
            <button 
              ...
              className={`md:hidden p-2 flex items-center gap-1 text-sm font-semibold rounded-lg transition-colors ${isScrolled ? 'text-black hover:bg-gray-100' : 'text-white'}`}
            >
          ...
          <div className="flex items-center justify-center w-1/3">
            <button onClick={onLogoClick} className="focus:outline-none" aria-label="Voltar para a página inicial">
              ...
              <img 
                src="/logodrop.PNG" 
                alt="DROP Logo"
                className={`w-auto h-8 transition-all duration-300 ease-in-out ${!isScrolled ? 'filter invert' : ''}`}
              />
              {/* ===== FIM DA ÁREA MODIFICADA ===== */}
            </button>
          </div>
          ...
          <div className="flex items-center justify-end w-1/3 space-x-4">
            ...
            <a href="#busca" className="hover:opacity-75 transition-opacity">
              <img src="/iconlupa.PNG" alt="Ícone de Busca" className={`w-6 h-6 ${!isScrolled ? 'filter invert' : ''}`} />
            </a>
            
            <button onClick={toggleCart} className={`relative hover:opacity-75 transition-opacity ${isScrolled ? 'text-black' : 'text-white'}`}>
