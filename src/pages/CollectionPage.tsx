  ...
  const ProductCard = ({ product }: { product: Product }) => (
    <Link to={`/produto/${product.id}`} className="group relative text-left cursor-pointer">
      ...
      <div className="mt-4">
        ...
        <h4 className="text-md md:text-lg font-bold text-black truncate font-serif">
          {product.name}
        </h4>
        <div className="mt-0 flex items-baseline gap-2">
          <p className="text-lg md:text-xl font-price font-bold text-black font-serif">
            R$ {product.price.toFixed(2).replace('.', ',')}
          </p>
          <p className="text-sm md:text-md font-medium text-gray-400 line-through font-serif">
